...
function corePlugin(opts = {}) {
  const config = {
    contextRegex: defaults.contextRegex,
    ...opts,
  };
...
function runPlugins(plugins, event, data, ...args) {
  let stop = false;

  return plugins.concat(corePlugin()).reduce((p, next) => {
    if (stop || !next[event]) return p;

    return p.then(data => {
      let ret = next[event](data, ...args);
...
function statiq(opts = {}) {
  ...
  return {
    ...
    async create(path, context, content) {
      ...
      document = await runPlugins(config.plugins, 'beforeCreate', document);
      ...
      runPlugins(config.plugins, 'afterCreate', document);
    ...
    async read(path) {
      ...
      document = await runPlugins(config.plugins, 'beforeRead', document);
      ...
      runPlugins(config.plugins, 'afterRead', document);
    ...
    async update(path, context, content) {
      ...
      document = await runPlugins(config.plugins, 'beforeUpdate', document, context, content);
      ...
      runPlugins(config.plugins, 'afterUpdate', document);
    ...
    async build(path) {
      ...
      document = await runPlugins(config.plugins, 'beforeBuild', document, template);
      runPlugins(config.plugins, 'afterBuild', document);
    ...
    async write(path) {
      ...
      document = await runPlugins(config.plugins, 'beforeWrite', document);

      if (!document) return;

      await writeFile(document.publishPath, document.content);

      runPlugins(config.plugins, 'afterWrite', document);
    ...
    async handleAssets() {
      const assets = await readdir(paths().assets);

      await mkdirp(paths().publish);

      return Promise.all(assets.map(async file => {
        let assetPath = join(paths().assets, file);
        let publishPath = join(paths().publish, file);
        let asset = { assetPath, publishPath };

        asset = await runPlugins(config.plugins, 'beforeAsset', asset);

        if (!asset) return;

        runPlugins(config.plugins, 'afterAsset', asset);
      }));
    },
