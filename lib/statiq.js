...
Statiq.prototype.config = function(options) {
    this._config = assign(this._config, options);
};
...
Statiq.prototype.index = function() {
    ...
    (function readdir(dir) {
        ...
        if (stat.isDirectory()) {
            ...
            var filename, document, locals = {}, sections = {};
            var match = src.match(config.localsRegex);

            if (match) {
                try {
                    locals = yaml.safeLoad(match[1]);
            ...
            filename = path.relative(srcPath, dir).replace(new RegExp(config.srcExt + '$'), (locals.dstExt || config.dstExt))
            locals.filename = path.basename(filename);
...
Statiq.prototype.parse = function() {
    ...
    index.forEach(function(file){
        var hash = assign({ document: file.document }, config.globals, file.locals, file.sections),
        ...
        index.forEach(function(f){
            if (config.ignoreRegex.test(path.basename(f.srcFile))) return;
            var dir = path.dirname(f.dstFile);
            !hash.index[dir] && (hash.index[dir] = []);
            hash.index[dir].push(assign({}, f.locals, {
        ...
        Promise.resolve()
            .then(function(){
                return config.templateParser(hash.template, hash);
            })
