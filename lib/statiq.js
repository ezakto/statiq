...
function statiq(opts = {}) {
  ...
  function runPlugins(event, initialData, ...args) {
    let stop = false;
    ...
    return config.plugins.concat(corePlugin(config)).reduce((p, next) => {
      ...
      return p.then(data => {
        ...
        return next[event].call(site, data, ...args);
      });
  ...
  const site = {
    ...
    async readAll() {
      const doclist = await readdirrec(paths().content, {
        filter: { ext: config.contentExtension },
      });

      return Promise.all(doclist.map(contentPath => {
