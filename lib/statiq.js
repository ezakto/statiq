...
class Statiq {
  ...
  loadFile(file) {
    ...
    const context = this.mergeContexts(dirname);
    const match = source.match(config.contextRegex);
    const blocks = {};
    ...
    if (match) {
      try {
        ...
        document = source.substr(match[0].length);
      } catch(e) {}
    }

    document = config.contentParser(document.replace(config.blocksRegex, (match, id, content) => {
      blocks[id] = config.contentParser(content.trim());
      return '';
    }).trim());

    return { filepath, filename, dirname, source, context, blocks, document };
  }
  ...
  buildFile(file) {
    ...
    const publishFilename = path.basename(file.filename, config.contentExtension) + config.publishExtension;
    const target = path.join(publishDir, file.dirname, publishFilename);
    const template = path.join(templatesDir, file.context.template || config.defaultTemplate);
    const context = Object.assign({}, file.context, file.blocks, {
    ...
    index.forEach(f => {
      ...
      context.index[f.dirname].push(Object.assign({}, f.context, {
        path: path.relative(file.dirname, path.join(f.dirname, path.basename(f.filename, config.contentExtension) + config.publishExtension)),
        current: file.filepath === f.filepath,
      }));
    });
