...
function simpleParser(template, document) {
  const data = Object.assign({}, document.context, { content: document.content });
...
class Statiq {
  ...
  create(file, context, content = '') {
    ...
    return mkdirp(path.dirname(sourcePath)).then(() => {
    ...
    return new Promise(resolve => {
      ...
      Object.keys(this._cachedDocuments).forEach(f => {
        ...
        context.index[dir].push(Object.assign({}, doc.context, {
          ...
          current: doc.sourcePath === document.sourcePath,
        }));
      });
      ...
      return new Promise((resolve, reject) => {
        fs.readFile(document.templatePath, 'utf8', (err, template) => {
          ...
          Promise.resolve()
            .then(() => config.templateParser(template, document))
            .then(data => {
              return mkdirp(path.dirname(document.publishPath)).then(() => {
   ...
   */
  buildAll() {
    return Promise.all(Object.keys(this._cachedDocuments).map(file => this.build(file)));
  }
  ...
  copyAsset(asset) {
    ...
    return mkdirp(path.dirname(publishPath)).then(() => {
      return config.assetHandler(sourcePath, publishPath);
    });
  }
   ...
   */
  run() {
    return this.clear().then(() => this.copy()).then(() => this.scan()).then(() => this.buildAll());
  }
}
